...
const saveUserToLocal = (user) => {
  if (user) {
    localStorage.setItem("provinceUser", JSON.stringify(user)); // Store as a simple string
  } else {
    localStorage.removeItem("provinceUser");
  }
};
...
const getUserFromLocal = () => {
  return JSON.parse(localStorage.getItem("provinceUser")) || null;
};
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("provinceTokens")) {
        db.createObjectStore("provinceTokens", { keyPath: "id" });
      }
    };
...
async function storeRefreshToken(token) {
  ...
  const transaction = db.transaction("provinceTokens", "readwrite");
  const store = transaction.objectStore("provinceTokens");
...
async function getRefreshTokenFromDb() {
  const db = await openDatabase();
  const transaction = db.transaction("provinceTokens", "readonly");
  const store = transaction.objectStore("provinceTokens");
...
async function deleteRefreshTokenFromDb() {
  const db = await openDatabase();
  const transaction = db.transaction("provinceTokens", "readwrite"); // Use readwrite for deletion
  const store = transaction.objectStore("provinceTokens");
